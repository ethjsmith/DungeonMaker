...
def randomshapes(numVertices,start,size,result, initialAngle, midpoints=[]):
    ...
    if result != 0: # this is SUPER bad lol, basically if this ISN'T the first run, then it creates a new midpoint to draw a new circle
        ...
        while counter > 10 or contained(tempo,midpoints): # tries 10 times to place the new point, not in proximity to other points. # This is inelegant, especially in denser maps
            ...
            s = start[0] + math.cos(initialAngle)*size*4*random.uniform(.1,2)
            v = start[1] + math.sin(initialAngle)*size*4*random.uniform(.1,2)
...
def sinplifySingle(ind,coords):
    print("todo")
def simplify(coords): # wtf
    for c in coords: # loop over each point
        intersects = 0
        for p,o in zip(coords,coords[1:]):  # loop over each coordinate pair
            # draw a line extending out forever, and check how many collisions there are
            if (c[1] >= p[1] and c[1] <= o[1]) or (c[1] >= o[1] and c[1] <= p[1]):
                intersects += 1
        # if intersects is even, then remove this point. HOW?
